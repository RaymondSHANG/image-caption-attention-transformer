...
class transformerDecoderLayerNoSelfAtt(nn.Module):
    def __init__(self, d_model: int, num_heads: int, feedforward_dim: int,
                 dropout: float):
        super(transformerDecoderLayerNoSelfAtt, self).__init__()
        """
        param:
        d_model:    features size.
                    int
    ...
    def forward(self, dec_inputs: Tensor, enc_outputs: Tensor,
        ...
        # average_attn_weights=False for multiheadAtt
        output2, attns = self.multihead_attn(
            dec_inputs, enc_outputs, enc_outputs, average_attn_weights=average_attn_weights)
        output = dec_inputs + self.multihead_dropout(output2)
        output = self.multihead_norm(output)
...
class DecoderWithTransformerCrossAtt(nn.Module):
    """
    ...
    def __init__(self,
        ...
        """
        super(DecoderWithTransformerCrossAtt, self).__init__()
        self.pad_id = pad_id
        ...
        ###############################   TransformerDecoder  with only 1 dec_enc cross attention layer  ##################################
        # Embedding layer + pos encoding
        decoder_layer = transformerDecoderLayerNoSelfAtt(d_model=embed_dim,
...
class DecoderWithTransformerOnlyDecoder(nn.Module):
    """
    Decoder with transformer
    The input is (N,W,H,embed_dim), which will firstly to (N,W*H,embed_dim) as input for the whole transformer
    The transformer itself will contain both encoder and decoder
    ...
    def __init__(self,
        ...
        :encoder_numpixel=196,this number should be the W*H value from encoder outputs
        """
        super(DecoderWithTransformerOnlyDecoder, self).__init__()
        self.pad_id = pad_id
...
class DecoderWithTransformerFull(nn.Module):
    """
    Decoder with transformer
    The input is (N,W,H,embed_dim), which will firstly to (N,W*H,embed_dim) as input for the whole transformer
    The transformer itself will contain both encoder and decoder
    ...
    def __init__(self,
        ...
        :encoder_numpixel=196,this number should be the W*H value from encoder outputs
        """
        super(DecoderWithTransformerFull, self).__init__()
        self.pad_id = pad_id
        ...
        self.enc_embed_relu = nn.ReLU(inplace=True)
        # MHA+norm
        self.enc_multi_head_attn = MultiheadAttention(embed_dim=embed_dim,
                                                      num_heads=num_heads_encoder,
                                                      dropout=enc_dropout)
        self.enc_layer_norm1 = nn.LayerNorm(embed_dim)
        # FFL,PoswiseFeedForwardNet(nn.Module),using conv1d, we also encode the position information
        # Based on:
        ...
        self.enc_conv1 = nn.Conv1d(in_channels=encoded_image_size*encoded_image_size,
                                   out_channels=enc_feedforward_dim,
                                   kernel_size=1)
        self.enc_conv2 = nn.Conv1d(in_channels=enc_feedforward_dim,
                                   out_channels=encoded_image_size*encoded_image_size,
                                   kernel_size=1)
        self.enc_relu = nn.ReLU(inplace=True)
        self.enc_dropout = nn.Dropout(p=enc_dropout)
        self.enc_layer_norm2 = nn.LayerNorm(embed_dim)
    ...
    def forward(self, encoder_out, encoded_captions, caption_lengths):
        ...
        enc_output2, _ = self.enc_multi_head_attn(encoder_out, encoder_out,
                                                  encoder_out)
        enc_output2 = enc_output2 + encoder_out
        enc_output2 = self.enc_layer_norm1(enc_output2)

        enc_output3 = self.enc_conv2(
            self.enc_relu(self.enc_conv1(enc_output2.permute(1, 0, 2))))
        enc_output3 = self.enc_dropout(enc_output3)  # type: Tensor
        enc_output = self.enc_layer_norm2(
            enc_output3.permute(1, 0, 2) + enc_output2)
        # after encoder, we have ([encode_size^2, batch_size, embed_dim])
        ###################################    Tansformer decoder    ##########################
