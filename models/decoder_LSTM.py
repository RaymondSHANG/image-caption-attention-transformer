...
class DecoderWithoutAttention(nn.Module):
    ...
    def forward(self, encoder_out, encoded_captions, caption_lengths):
        ...
        predictions = torch.zeros(batch_size, max(
            decode_lengths), vocab_size).to(device)
        alphas = torch.ones(batch_size, max(decode_lengths), num_pixels).to(device)
        ...
        for t in range(max(decode_lengths)):
            ...
            # Here,attention_weighted_encoding att weight for each channel,
            # alpha, att weight for each pixel
            # gating scalar, (batch_size_t, encoder_dim)
            gate = self.sigmoid(self.f_beta(h[:batch_size_t]))
            # attention_weighted_encoding = gate * attention_weighted_encoding
