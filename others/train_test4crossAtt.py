...
from models_test4crossAtt import Encoder, DecoderWithTransformer
...
model_name = 'model_trans6'  # model1:CNN, Transformer_full
...
# 'BEST_checkpoint_model5_coco_5_cap_per_img_5_min_word_freq.pth.tar'
# 'BEST_checkpoint_model_trans6_coco_5_cap_per_img_5_min_word_freq.pth.tar'
checkpoint = 'BEST_checkpoint_model_trans6_coco_5_cap_per_img_5_min_word_freq.pth.tar'
...
def alpha_reg(attns: Tensor, alpha_c) -> Tensor:
    ...
    # layers?
    # alpha_trans_c = alpha_c / (args.n_heads * args.decoder_layers)
...
def main():
    ...
    scheduler_decoder = torch.optim.lr_scheduler.ReduceLROnPlateau(
        decoder_optimizer)
    scheduler_encoder_check = False
    if encoder_optimizer is not None:
        scheduler_encoder_check = True
        scheduler_encoder = torch.optim.lr_scheduler.ReduceLROnPlateau(
            encoder_optimizer)
    # Custom dataloaders
    ...
    for epoch in range(start_epoch, epochs):
        if False:  # epoch == 10:  # epoch == 10:
            # Start tune
            if True:  # best_epoch != 9:
                # Reload Best check model
                del decoder
                del encoder
                del decoder_optimizer
                torch.cuda.empty_cache()
                checkpoint = torch.load(
                    checkpoint_folder+"/"+'BEST_checkpoint_model_trans6_coco_5_cap_per_img_5_min_word_freq.pth.tar')
                start_epoch = checkpoint['epoch'] + 1
                epochs_since_improvement = checkpoint['epochs_since_improvement']
                best_bleu4 = checkpoint['bleu-4']
                decoder = checkpoint['decoder']
        ...
        trainloss, trainacc = train(train_loader=train_loader,
                                    ...
                                    epoch=epoch)
        scheduler_decoder.step(trainloss)
        if scheduler_encoder_check:
            scheduler_encoder.step(trainloss)
...
def train(train_loader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer, epoch):
    ...
    for i, (imgs, caps, caplens) in enumerate(train_loader):
        ...
        # (args.n_heads * args.decoder_layers)
        alpha_trans_c = alpha_c / (2 * 1)
